...
function doReadDir(path, options, callback, shared) {
	...
	function doStat(fullPath, res) {
		...
		options.stat.fn(fullPath, function (err, stat) {
			...
			if (options.content.enabled && !isDirectory) {
				doReadContent(fullPath, stat, res);
			}
	...
	function doReadContent(fullPath, stat, res) {
		if (options.content.maxSize && options.content.maxSize < stat.size) {
			return;
		}

		var asText = options.content.asText === true;
		if (!asText && options.content.asText) {
			options.content.asText.forEach(function (test) {
				if (libTools.isFunction(test)) {
					if (test(fullPath, stat)) {
						asText = true;
						return false;
					}
				}
		...
		waitCount++;
		var encoding = asText ? "utf-8" : null;
		options.content.fn(fullPath, encoding, function (err, content) {
			if (err) {
