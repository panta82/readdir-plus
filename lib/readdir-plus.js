...
function doReadDir(path, options, callback, shared) {
    ...
    files.forEach(function(file) {
      ...
      if (shared.details) {
        ...
        switch (options.return) {
          case libVars.RETURN_TYPES.names:
            res = file;
            break;
          case libVars.RETURN_TYPES.relativePaths:
            res = relativePath;
            break;
          default:
            res = fullPath;
  ...
  function doHandleError(err, name, res) {
    switch (options[name].errorStrategy) {
      ...
      default:
        onWaitDone(
          new Error("Unknown error strategy: " + options[name].errorStrategy)
        );
        return true;
    }
    onWaitDone();
    ...
    options.stat.fn(fullPath, function(err, stat) {
      ...
      var isDirectory = stat.isDirectory(),
        fileType = isDirectory
          ? libVars.FILE_TYPES.directory
          : stat.isFile()
          ? libVars.FILE_TYPES.file
          : stat.isSymbolicLink()
          ? libVars.FILE_TYPES.symbolicLink
          : stat.isBlockDevice()
          ? libVars.FILE_TYPES.blockDevice
          : stat.isCharacterDevice()
          ? libVars.FILE_TYPES.characterDevice
          : stat.isFIFO()
          ? libVars.FILE_TYPES.FIFO
          : stat.isSocket()
          ? libVars.FILE_TYPES.socket
          : null;
      if (shared.details) {
      ...
      if (
        (!detailedFilters && doFilter(res, options.filter)) ||
        (detailedFilters &&
          doFilter(res, options.filter.any) &&
          doFilter(res, options.filter[fileType]))
      ) {
        results.push(res);
        included = true;
      }
      ...
      if (
        ...
        isDirectory &&
        (!options.excludeDirs ||
          options.excludeDirs.indexOf(res.relativePath) === -1)
      ) {
        doRecurse(fullPath, res, included);
      }

      if (
        options.content.enabled &&
        shared.details &&
        fileType === libVars.FILE_TYPES.file
      ) {
  ...
  function doReadContent(res, stat) {
    ...
    if (options.content.asText === true || doTest(options.content.asText)) {
      ...
      options.content.asBinary === true ||
      doTest(options.content.asBinary)
    ) {
      encoding = null;
    }
    if (encoding === false) {
  ...
  function doRecurse(fullPath, directoryResult, included) {
    waitCount++;
    ...
    function doRecurseCallback(err, innerResults) {
      if (err) {
        return doHandleError(err, "readdir", directoryResult);
      }
      ...
      if (options.tree) {
        if (!shared.details) {
          var resHash = {};
          switch (options.return) {
            case libVars.RETURN_TYPES.fullPaths:
              resHash.path = directoryResult;
              break;
            case libVars.RETURN_TYPES.names:
              resHash.name = directoryResult;
              break;
            case libVars.RETURN_TYPES.relativePaths:
              resHash.relativePath = directoryResult;
              break;
          }
          directoryResult = resHash;
        }
        directoryResult.content = innerResults;
        if (!included) {
          // We want to always include directories, because they will hold items in tree view
...
function readdirPlus(path, userOptions, callback) {
  ...
  function normalizeSection(section) {
    ...
    if (
      libTools.isAnonObject(section) &&
      !libTools.isBoolean(section.enabled)
    ) {
  ...
  function normalizeArray(target, source, property) {
    ...
    if (!libTools.isArray(target[property])) {
      ...
      libTools.isArray(target[property][0])
    ) {
...
readdirPlus.FILE_TYPES = libVars.FILE_TYPES;
