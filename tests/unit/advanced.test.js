...
module.exports = {
  setUp: libTools.execOrDie.bind(
    null,
    libPath.resolve(__dirname, "../root/setup.sh")
  ),
  tearDown: libTools.execOrDie.bind(
    null,
    libPath.resolve(__dirname, "../root/teardown.sh")
  ),
  ...
  canProduceTreeWithDetailsReturnType: function(test) {
    ...
    readdir(rootAdvanced, { tree: true, stat: false }, function(err, results) {
      ...
      results.forEach(function(result) {
        if (result.name === "sub1") {
          test.equal(result.content.length, 3);
          //test.equal(result.content.find(function (x) { return x.name === "sub2" }).content.length, 1);
  ...
  canProduceTreeWithFiltering: function(test) {
    test.expect(2);
    readdir(rootAdvanced, { tree: true, filter: { directory: true } }, function(
      err,
      results
    ) {
      test.equal(err, null);
  ...
  canProduceTreeWithOtherReturnTypes: function(test) {
    ...
    readdir(
      rootAdvanced,
      { tree: true, stat: false, return: "names" },
...
// NOTE: Used to run only one test
// for (var key in module.exports) {
//   if (
//     key !== "tearDown" &&
//     key !== "setUp" &&
//     key !== "canProduceTreeWithDetailsReturnType"
//   ) {
